...
class Client:
    ...
    def create_table(data: str):
        ...
        max_len_date, max_len_description, max_len_withdrawals = 4, 15, 11
        ...
        for d in data:
            ...
            if d['Withdrawals']:
                total_balance -= float(d['Withdrawals'][1:])
                total_withdrawals += float(d['Withdrawals'][1:])
            if d['Deposits']:
                total_balance += float(d['Deposits'][1:])
                total_deposits += float(d['Deposits'][1:])
        ...
        print("|" + " " * (max_len_date + 1) + "|Previous balance" + " " * (
                max_len_description - len("Previous balance") + 1) + "|" + " " * (
                      max_len_withdrawals + 1) + "|" +
              " " * (max_len_deposits + 1) +
              "|$0" + " " * (
                      max_len_balance - 1) + "|")
    ...
    def send_data(self, data: str, com=None):
        """Отправка сообщения серверу"""
        if com == 'del':
            # если заканчиваем сесию
            ...
            try:
                ...
                while not data.endswith(b"\n\n"):  # считываем пока не встретим признак конца ответного сообщения
                    try:
                        ...
                        raise ClientError("Connection fails.")
...
if __name__ == "__main__":
    ...
    print(f"{datetime.now()}: Session start.")  # фиксируем фремя соединения с сервером
    ...
    try:
        while True:  # начниаем ссесию, бесконечно опрашиваем пользователя пока он не решит разорвать соединение
            ...
            if message == "--exit":  # выходим из ссесии
                client.send_data("delete", com='del')  # команда серверу удалить данные сессии
                break
