...
bills = {}
patterns = {"deposit":
                r'deposit --client="(\w+ \w+)" --amount=(\d+) --description="([\w\s]+)"',
            "withdraw":
                r'withdraw --client="(\w+ \w+)" --amount=(\d+) --description="([\w\s]+)"',
            "show_bank_statement":
...
class ClientServerProtocol(asyncio.Protocol):
    ...
    def deposit(self, uid: str, groups: tuple):
        ...
        client, amount, description = groups
        if uid in bills:
            if client in bills[uid]:
                bills[uid][client].append({
                    ...
                    "Withdrawals": "",
                    "Deposits": f"${amount}",
                    "Balance": f"${int(amount) + int(bills[uid][client][-1]['Balance'][1:])}"
                })
            else:
                bills[uid][client] = [{
                    "Date": datetime.now(),
    ...
    def withdraw(self, uid: str, groups: tuple):
        ...
        client, amount, description = groups
        if uid in bills:
            withdraw = int(bills[uid][client][-1]['Balance'][1:]) - int(amount)
            if withdraw >= 0:
                if client in bills[uid]:
                    bills[uid][client].append({
                        "Date": datetime.now(),
